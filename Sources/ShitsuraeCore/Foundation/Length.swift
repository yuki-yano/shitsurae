...
public enum LengthUnit: Equatable, Sendable {
    case pt
    case percent
    case ratio
    case px
...
public struct ParsedLength: Equatable, Sendable {
...
public struct ResolvedFrame: Codable, Equatable, Sendable {
    public let x: Double
    public let y: Double
...
public enum LengthParser {
    ...
    /// Resolves a frame definition against a basis rect. x/y are offsets
    /// from the basis origin (v2: the origin is added, so frames land inside
    /// the visible area even with a menu bar offset or a non-zero display
    /// origin — required for the multi-display extension).
